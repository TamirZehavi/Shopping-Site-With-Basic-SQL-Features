...
if($_POST["id"]==$currentItemID) //if the user has not updated the ID, let him proceed
{
...
else if(isset($_POST["id"]) && is_numeric($_POST["id"]) && $_POST["id"]>0 && !strpos($_POST["id"],'.'))
{
...
$update=$DBObject->Update($currentItemID, $id, $name, $description, $price, $picture); //preform insert operation
...
if ($update){
    ...
    echo json_encode(array("statusCode"=>"Update failed"));
}
